...
class FASTrainer(BaseTrainer):
    def __init__(self, cfg, network, optimizer, criterion, lr_scheduler, device, trainloader, valloader):
        super(FASTrainer, self).__init__(cfg, network, optimizer, criterion, lr_scheduler, device, trainloader, valloader)

        self.network = self.network.to(device)

        self.train_loss_metric = AvgMeter( name='Loss/train', num_iter_per_epoch=len(self.trainloader), per_iter_vis=True)
        self.train_acc_metric = AvgMeter( name='Accuracy/train', num_iter_per_epoch=len(self.trainloader), per_iter_vis=True)

        self.val_loss_metric = AvgMeter( name='Loss/val', num_iter_per_epoch=len(self.valloader))
        self.val_acc_metric = AvgMeter( name='Accuracy/val', num_iter_per_epoch=len(self.valloader))
    ...
    def train_one_epoch(self, epoch):
        ...
        print('Epoch: {}, loss: {}, acc: {}'.format(epoch,self.train_loss_metric.avg, self.train_acc_metric.avg))
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            ...
            print('Epoch_Val: {}, loss: {}, acc: {}'.format(epoch, self.val_loss_metric.avg, self.val_acc_metric.avg))
